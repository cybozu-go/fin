...
var (
	...
	ErrChecksumMismatch             = csumio.ErrChecksumMismatch
	csumZero                 uint64
)

const defaultRawChecksumChunkSize = 64 * 1024
		...
		case 'w', 'z': // UPDATED DATA or ZERO DATA
			...
			if !isDstBlockDevice && offset+length > uint64(fileSize) {
				...
				if enableChecksumVerification {
					...
					neededChunks := uint64(fileSize) / rawChecksumChunkSize
					if uint64(fileSize)%rawChecksumChunkSize != 0 {
						neededChunks++
					}
