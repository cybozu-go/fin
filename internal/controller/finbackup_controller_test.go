...
var _ = Describe("FinBackup Controller Reconcile Test", Ordered, func() {
	...
	// CSATEST-1607
	// Description:
	//   Prevent backups from being created for PVCs that are not in the same Ceph cluster.
	//
	// Arrange:
	//   - Create another namespace and storage class.
	//   - Create PVC in the other namespace.
	//
	// Act:
	//   - Create a FinBackup targeting a PVC in a different CephCluster
	//
	// Assert:
	//   - The reconcile process does not return a error.
	//   - The reconcile process does not create a backup job.
	//   - The FinBackup is not marked as ready.
	Context("PVC from a different CephCluster should not be processed", func() {
		var pvc1 *corev1.PersistentVolumeClaim
		var pv1 *corev1.PersistentVolume
		var finbackup *finv1.FinBackup

		BeforeAll(func(ctx SpecContext) {
			By("creating another namespace and storage class")
			otherNamespace := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "test-namespace-1"}}
			...
			By("creating PVC in the other namespace")
			pvc1, pv1 = NewPVCAndPV(otherStorageClass, otherNamespace.Name, "test-pvc-1", "test-pv-1", rbdImageName)
			Expect(k8sClient.Create(ctx, pvc1)).Should(Succeed())
			Expect(k8sClient.Create(ctx, pv1)).Should(Succeed())

			By("creating a FinBackup targeting a PVC in a different CephCluster")
			finbackup = NewFinBackup(namespace, "test-fin-backup-1", pvc1.Name, pvc1.Namespace, "test-node")
