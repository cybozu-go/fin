...
var _ = Describe("FinBackup Controller integration test", Ordered, func() {
	...
	//
	// Assert:
	//   - A FinBackup on an existing node records that node's UID in its status.
	...
	//   - One marked corrupted still records the UID, so a node recreated under the same
	//     name can be told apart from the original.
	...
	Describe("a FinBackup whose backup destination node is unavailable", func() {
		...
		It("should record the UID of its node", func(ctx SpecContext) {
			node = &corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: "node-uid-node"}}
			Expect(k8sClient.Create(ctx, node)).Should(Succeed())
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(node), node)).To(Succeed())

			finbackup = NewFinBackup(workNamespace, "fb-node-uid", pvc.Name, pvc.Namespace, node.Name)
			Expect(k8sClient.Create(ctx, finbackup)).Should(Succeed())

			Eventually(func(g Gomega) {
				var got finv1.FinBackup
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(finbackup), &got)).To(Succeed())
				g.Expect(got.Status.NodeUID).To(Equal(node.UID))
			}, "10s", "1s").Should(Succeed())
		})
		...
		It("should record the node UID on a FinBackup marked corrupted", func(ctx SpecContext) {
			node = &corev1.Node{ObjectMeta: metav1.ObjectMeta{Name: "corrupt-node-uid-node"}}
			Expect(k8sClient.Create(ctx, node)).Should(Succeed())
			Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(node), node)).To(Succeed())

			finbackup = NewFinBackup(workNamespace, "fb-corrupt-node-uid", pvc.Name, pvc.Namespace, node.Name)
			Expect(k8sClient.Create(ctx, finbackup)).Should(Succeed())
			Eventually(func(g Gomega) {
				var got finv1.FinBackup
				g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(finbackup), &got)).To(Succeed())
...
func Test_ensureNodeUIDRecorded(t *testing.T) {
	tests := []struct {
		name     string
		recorded types.UID
		want     types.UID
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r := &FinBackupReconciler{Client: c}

			require.NoError(t, r.ensureNodeUIDRecorded(context.Background(), backup, "uid-live"))
			require.Equal(t, tt.want, backup.Status.NodeUID, "in memory")

			var got finv1.FinBackup
			require.NoError(t, c.Get(context.Background(), client.ObjectKeyFromObject(backup), &got))
			require.Equal(t, tt.want, got.Status.NodeUID, "persisted")
		})
	}
}
...
func Test_reconcileDeleteOnGoneNode(t *testing.T) {
	tests := []struct {
		...
		{
			// The image is unreachable once the PVC is gone, so it is not touched at all
			// and the retry period never comes into play.
			name:          "a PVC that is gone, which leaves the image unreachable",
			deletedAgo:    goneNodeSnapshotRetryPeriod / 2,
			pvcDeleted:    true,
			wantErr:       false,
			wantFinalizer: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := r.reconcileDeleteOnGoneNode(context.Background(), backup, tt.pvcDeleted)
			if tt.wantErr {
				require.Error(t, err)
			} else {
...
func Test_Reconcile_goneNodeOwnership(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The gone-node path deletes jobs before looking at anything else, so it has to
			// stay behind the ownership check just as the FinBackup deletion does.
			job := &batchv1.Job{ObjectMeta: metav1.ObjectMeta{
				Namespace: cephNamespace,
				Name:      backupJobName(backup),
			}}

			objs := []client.Object{pvc, pv, backup, job}
			if tt.noPVC {
				objs = []client.Object{backup, job}
			}
			if tt.storageSC != nil {
			...
			require.Equal(t, tt.wantDeleted, k8serrors.IsNotFound(err), "backup job deleted?")
		})
	}
}
