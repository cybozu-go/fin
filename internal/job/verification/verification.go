...
func (v *Verification) doVerify() error {
	...
	} else if instantVerifyImageExists, err := v.doesInstantVerifyImageExist(); err != nil {
		return fmt.Errorf("failed to check instant_verify.img existence: %w", err)
	} else if !instantVerifyImageExists {
	...
	if shouldFsck {
		...
		if err := v.fsckRepo.Run(v.nlvRepo.GetInstantVerifyImagePath()); err != nil {
			if errors.Is(err, model.ErrFsckFailed) {
				// wrap ErrFsckFailed to return exit code 2
