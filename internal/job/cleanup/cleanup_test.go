...
func assertDiffDirDoesNotExist(t *testing.T, nlvRoot string, snapID int) {
	...
	assert.NoFileExists(t, filepath.Join(nlvRoot, "diff", strconv.Itoa(snapID)))
}
...
func TestCleanup_OnApplyingDiffForFullBackup_Success(t *testing.T) {
	...
	//   3. Create the diff directory for full backup.
	//   4. Set backup_metadata table means applying diff.
	//
	// Act:
	//   Run the cleanup process.
	//
...
func TestCleanup_FullBackupExistAndNoOngoingIncrementalBackup_Success(t *testing.T) {
	...
	//   2. Set backup_metadata table means that full backup exists,
	//      incremental backup doesn't exist and no ongoing incremental backup.
	//
	// Act:
	//   Run the cleanup process.
	//
...
func TestCleanup_OnIncrementalBackup_Success(t *testing.T) {
	...
	//   3. Set backup_metadata table means there is an on-going incremental backup.
	//
	// Act:
	//   Run the cleanup process.
	//
...
func TestCleanup_BothFullBackupAndIncrementalBackupExist_Success(t *testing.T) {
	...
	// Arrange:
	//   1. Create raw.img, pvc.yaml, and pv.yaml
	//   2. Set backup_metadata table means when both full backup and incremental backup exist.
	//
	// Act:
	//   Run the cleanup process.
	//
...
func TestCleanup_PVDUIDMismatch_Failure(t *testing.T) {
	// CSATEST-1598
	// Description:
	...
	// Arrange:
	//   1. Create raw.img, pvc.yaml, and pv.yaml
	//   2. Set backup_metadata table means when full backup exists
	//      and no ongoing incremental backup.
	//
	// Act:
