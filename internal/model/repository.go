...
type NodeLocalVolumeRepository interface {
	...
	// RemoveInstantVerifyImage removes the instant verify image file.
...
// FsckRepository is an interface for running fsck on a filesystem image.
type FsckRepository interface {
	// Run runs fsck on the given image path. It returns nil if the filesystem is clean.
	// If the filesystem has errors that cannot be fixed without user intervention, it returns `model.ErrFsckFailed`.
	Run(imagePath string) error
}
