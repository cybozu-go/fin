...
//  3. A FinBackup1 that references the PVC is created.
...
//
// Assert:
//   - The FinBackup condition "ReadyToUse" becomes "True".
//   - On the Fin node for the FinBackup, the backup directory has:
...
func incrementalBackupTestSuite() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		By("creating a Pod")
		devicePath = "/data"
		pod, err = GetPod(pvcNamespace, "test-pod", "test-pvc", "ghcr.io/cybozu/ubuntu:24.04", devicePath)
		Expect(err).NotTo(HaveOccurred())
		Expect(CreatePod(ctx, k8sClient, pod)).NotTo(HaveOccurred())
		Expect(WaitForPodReady(ctx, k8sClient, pvcNamespace, "test-pod", 2*time.Minute)).NotTo(HaveOccurred())
		...
		finbackup, err = GetFinBackup(rookNamespace, "finbackup-test-1", pvcNamespace, pvc.Name, "minikube-worker")
		Expect(err).NotTo(HaveOccurred())
		Expect(CreateFinBackup(ctx, ctrlClient, finbackup)).NotTo(HaveOccurred())
		Expect(WaitForFinBackupReady(ctx, ctrlClient, rookNamespace, finbackup.Name, 1*time.Minute)).NotTo(HaveOccurred())
	...
	It("should create an incremental backup", func(ctx SpecContext) {
		...
		finbackup2, err := GetFinBackup(rookNamespace, "finbackup-test-2", pvcNamespace, pvc.Name, "minikube-worker")
		Expect(err).NotTo(HaveOccurred())
		Expect(CreateFinBackup(ctx, ctrlClient, finbackup2)).NotTo(HaveOccurred())
		Expect(WaitForFinBackupReady(ctx, ctrlClient, rookNamespace, finbackup2.Name, 1*time.Minute)).NotTo(HaveOccurred())
